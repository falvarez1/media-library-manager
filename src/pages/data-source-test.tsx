...
interface UseDelayedFetchReturn {
  data: any;
  loading: boolean;
  error: any;
  fetchWithDelay: () => Promise<void>;
}

const DataSourceTestPage: React.FC = () => {
  const [activeTest, setActiveTest] = useState<string>('read');
  const [selectedEntity, setSelectedEntity] = useState<string>('media');
  const [testResult, setTestResult] = useState<any>(null);
  const [errorSimulation, setErrorSimulation] = useState<ErrorSimulationState>({
    enabled: false,
    rate: 0.5,
    originalRate: 0
  });
  ...
  }, [errorSimulation.enabled, errorSimulation.rate, errorSimulation.originalRate]);

  const handleTestChange = (test: string): void => {
    setActiveTest(test);
    setTestResult(null);
  };

  const handleEntityChange = (entity: string): void => {
                  ...
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="relative inline-block w-10 mr-2 align-middle select-none">
                      <input 
                        ...
                        onChange={(e: ChangeEvent<HTMLInputElement>) => setErrorSimulation(prev => ({
                          ...prev,
...
function getTestTitle(testType: string, entityType: string): string {
...
const TestButton: React.FC<TestButtonProps> = ({ label, isActive, onClick }) => {
  return (
    <button 
...
const TestPanel: React.FC<TestPanelProps> = ({ testType, entityType, onResult }) => {
  switch(testType) {
    case 'read':
...
// Read Test Component
const ReadTest: React.FC<ReadTestProps> = ({ entityType, onResult }) => {
  const [params, setParams] = useState<TestParams>({ page: 1, pageSize: 5 });
...
const CreateTest: React.FC<CreateTestProps> = ({ entityType, onResult }) => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      switch(entityType) {
        case 'media':
          response = await api.media.createMedia(formValues as any);
          break;
        case 'folders':
          response = await api.folders.createFolder(formValues as any);
          break;
        case 'collections':
          response = await api.collections.createCollection(formValues as any);
          break;
        case 'tags':
          response = await api.tags.createTag(formValues as any);
          break;
      }
      ...
      setError(err);
      onResult({ error: (err as Error).message || 'Creation failed' });
    } finally {
      setCreating(false);
    }
  };
...
const UpdateTest: React.FC<UpdateTestProps> = ({ entityType, onResult }) => {
  ...
  const fetchItem = async (): Promise<void> => {
    ...
    try {
      ...
      switch(entityType) {
        case 'media':
          response = await api.media.getMediaById(id as any);
          break;
        case 'folders':
          response = await api.folders.getFolderById(id as any);
          break;
        case 'collections':
          response = await api.collections.getCollectionById(id as any);
          break;
        case 'tags':
          response = await api.tags.getTagById(id as any);
          break;
        default:
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      switch(entityType) {
        case 'media':
          response = await api.media.updateMedia(id as any, formValues);
          break;
        case 'folders':
          response = await api.folders.updateFolder(id as any, formValues);
          break;
        case 'collections':
          response = await api.collections.updateCollection(id as any, formValues);
          break;
        case 'tags':
          response = await api.tags.updateTag(id as any, formValues);
          break;
      }
      ...
      setError(err);
      onResult({ error: (err as Error).message || 'Update failed' });
    } finally {
      setUpdating(false);
    }
  };
...
const DeleteTest: React.FC<DeleteTestProps> = ({ entityType, onResult }) => {
  const [id, setId] = useState<string>('');
  ...
  const handleDelete = async (): Promise<void> => {
    ...
    try {
      ...
      switch(entityType) {
        case 'media':
          response = await api.media.deleteMedia(id as any);
          break;
        case 'folders':
          response = await api.folders.deleteFolder(id as any);
          break;
        case 'collections':
          response = await api.collections.deleteCollection(id as any);
          break;
        case 'tags':
          response = await api.tags.deleteTag(id as any);
          break;
      }
      ...
      setError(err);
      onResult({ error: (err as Error).message || 'Deletion failed' });
    } finally {
      setDeleting(false);
    }
  };
...
const LoadingTest: React.FC<LoadingTestProps> = ({ entityType, onResult }) => {
  ...
  const useDelayedFetch = (): UseDelayedFetchReturn => {
    ...
    const fetchWithDelay = async (): Promise<void> => {
      ...
      try {
        ...
        setError(err);
        onResult({ error: (err as Error).message || 'Fetch failed' });
      } finally {
        setLoading(false);
      }
    };
      ...
      <div className="mb-4">
        <div className="flex items-center space-x-2 mb-2">
          <input
            ...
            onChange={(e: ChangeEvent<HTMLInputElement>) => setSimulateDelay(e.target.checked)}
        ...
        {simulateDelay && (
          <div className="flex items-center space-x-2 mb-4">
            <input
              ...
              step="500"
              value={delayTime}
              onChange={(e: ChangeEvent<HTMLInputElement>) => setDelayTime(parseInt(e.target.value))}
              className="w-1/2"
            />
...
const ErrorTest: React.FC<ErrorTestProps> = ({ entityType, onResult }) => {
  const [errorType, setErrorType] = useState<string>('network');
  const { dataSource } = useDataSource();
  
  const simulateError = async (): Promise<Error | void> => {
    try {
      let error: Error & { name?: string; status?: number; details?: any };
      ...
      onResult({ error: { message: error.message, name: error.name, status: error.status } });
      return error;
    }
  };
  
  // Function to get an appropriate error handling component
  const getErrorComponent = (errorType: string): ReactNode => {
    switch(errorType) {
      case 'network':
        return (
    ...
    <div>
      <div className="mb-4">
        ...
        <div className="mb-4">
          ...
          <select
            value={errorType}
            onChange={(e: ChangeEvent<HTMLSelectElement>) => setErrorType(e.target.value)}
            className="w-full p-2 border border-gray-300 rounded-md"
          >
...
function getEditableFields(entityType: string, item: any): Record<string, any> {
  if (!item) return {};
