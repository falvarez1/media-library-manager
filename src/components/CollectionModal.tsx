...
interface CollectionModalProps extends BaseComponentProps {
  isOpen: boolean;
  onClose: () => void;
  title?: string;
...
const CollectionModal: React.FC<CollectionModalProps> = ({ 
  ...
  const [parentId, setParentId] = useState<CollectionId | null>(null);
  ...
  // Event handlers
  const handleNameChange: EventHandler<ChangeEvent<HTMLInputElement>> = (e) => {
    setName(e.target.value);
  };

  const handleDescriptionChange: EventHandler<ChangeEvent<HTMLTextAreaElement>> = (e) => {
    setDescription(e.target.value);
  };

  const handleParentIdChange: EventHandler<ChangeEvent<HTMLSelectElement>> = (e) => {
    setParentId(e.target.value === '' ? null : e.target.value as CollectionId);
  };

  const handleColorButtonClick = (colorOption: string): void => {
    setColor(colorOption);
  };

  const handleColorInputChange: EventHandler<ChangeEvent<HTMLInputElement>> = (e) => {
    setColor(e.target.value);
  };

  const handleIsSharedChange: EventHandler<ChangeEvent<HTMLInputElement>> = (e) => {
    setIsShared(e.target.checked);
  };

  const handleFormSubmit: EventHandler<FormEvent<HTMLFormElement>> = (e) => {
    handleSubmit(e);
  };

  const handleSubmitButtonClick: EventHandler<MouseEvent<HTMLButtonElement>> = (e) => {
    handleSubmit(e);
  };
  ...
  return (
    ...
    >
      <div className="w-full max-w-md bg-white rounded-lg shadow-xl overflow-hidden max-h-screen flex flex-col">
        ...
        {/* Form */}
        <form onSubmit={handleFormSubmit} className="p-4 flex-1 overflow-y-auto">
          {/* Form error */}
          {formError && (
