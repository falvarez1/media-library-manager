 ...
 */
const getFileIcon = (type: MediaType): React.JSX.Element => {
  switch(type) {
    case 'image':
      return (
    ...
    case 'archive':
      return (
        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-archive">
 ...
 */
interface MediaItemProps extends BaseComponentProps {
  /** The media item data */
  item: MediaItemType;
  ...
  /** Handler for quick view button click */
  onQuickView?: (id: MediaId) => void;
  /** Handler for star toggle */
  onToggleStar?: (id: MediaId) => void;
  /** Handler for favorite toggle */
  onToggleFavorite?: (id: MediaId) => void;
}
...
/**
 * MediaItem component for displaying media items in grid or list view
 */
const MediaItem: React.FC<MediaItemProps> = ({ 
  ...
  // Handle star toggle
  const handleToggleStar = (e: MouseEvent<HTMLButtonElement>): void => {
    e.stopPropagation();
    if (onToggleStar) onToggleStar(item.id);
  };
  
  // Handle favorite toggle
  const handleToggleFavorite = (e: MouseEvent<HTMLButtonElement>): void => {
    e.stopPropagation();
